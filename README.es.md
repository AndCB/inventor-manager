...
## Implementación (nivel gratuito)

Esta arquitectura se implementa de forma gratuita, completamente fuera de AWS:

- **Frontend**: [Vercel](https://vercel.com) en el nivel gratuito (compilación estática de Vite).
- **Backend**: [Azure Container Apps](https://azure.microsoft.com/es-mx/products/container-apps/) en el plan de consumo gratuito (contenedor Docker).
- **Base de datos**: [oferta gratuita de Azure SQL Database](https://learn.microsoft.com/es-mx/azure/azure-sql/database/free-offer) (SQL Server administrado, gratuito para siempre dentro de los límites).

> Se requiere una cuenta de Azure. Una cuenta gratuita estándar pide una tarjeta de crédito al registrarse para verificación (nunca se cobra mientras permanezca dentro de los niveles gratuitos). Los estudiantes pueden usar la ruta sin tarjeta de Azure for Students (regístrese a través de GitHub Education); tenga en cuenta que la oferta gratuita de SQL no está disponible en el nivel `Azure for Students Starter`. Cualquiera que sea la ruta, confirme que la base de datos muestra el nivel de oferta gratuita y mantenga la opción por defecto `Auto-pause the database until next month`.

### 1. Crear la base de datos gratuita

1.  Inicie sesión en el [portal de Azure](https://portal.azure.com) y cree una nueva **SQL Database**.
2.  Elija el nivel de precios de la **oferta gratuita** (100,000 vCore-segundos y 32 GB al mes, gratuito para siempre). Confirme que el nivel de precios muestra la oferta gratuita antes de crear la base de datos y mantenga el comportamiento por defecto `Auto-pause the database until next month`.
3.  Cree un servidor con un inicio de sesión de administrador y una contraseña segura.
4.  En la configuración de **Redes** del servidor, active **Permitir que los servicios y recursos de Azure accedan a este servidor** (la API se ejecutará dentro de Azure, y es esto lo que le permite conectarse).
5.  Copie la cadena de conexión. Tiene esta forma: `Server=tcp:<servidor>.database.windows.net,1433;Database=Inventory;User Id=<usuario>;Password=<contraseña>;Encrypt=True`.

La base de datos gratuita se pausa automáticamente tras la inactividad. La API reintenta las conexiones al iniciar y durante las consultas, por lo que la reactivación es transparente.

### 2. Compilar y subir el contenedor del backend

Desde la raíz del repositorio (necesita una cuenta gratuita de [Docker Hub](https://hub.docker.com)):

```bash
docker build -f backend/Dockerfile -t <su-usuario-dockerhub>/inventory-api:latest .
docker push <su-usuario-dockerhub>/inventory-api:latest
```

### 3. Implementar la API en Azure Container Apps

1.  Cree una **Container App** y elija el **plan de consumo gratuito**.
2.  Apúntela a su imagen (`<su-usuario-dockerhub>/inventory-api:latest`) y defina el **puerto de destino en 8080**.
3.  Habilite la entrada HTTPS y acepte tráfico desde cualquier lugar.
4.  Agregue estas variables de entorno:

    | Variable | Valor |
    | --- | --- |
    | `ConnectionStrings__DefaultConnection` | Su cadena de conexión de Azure SQL del paso 1 |
    | `JWT__Key` | Un secreto fuerte y único (reemplace la clave de desarrollo por defecto) |
    | `CORS__AllowedOrigins` | `https://<su-app-vercel>.vercel.app` |
    | `ASPNETCORE_ENVIRONMENT` | `Production` |

5.  Implemente. En el primer inicio la aplicación aplica las migraciones y agrega los 73 artículos de ejemplo automáticamente.

Pruebe la API en `https://<su-url-de-app>/api/inventory`.

> Azure Container Apps usa una comprobación de salud TCP por defecto, que funciona tal cual. Si configura una sonda HTTP, apúntela a `/api/inventory` (la ruta raíz devuelve 404).

### 4. Implementar el frontend en Vercel

1.  Importe este repositorio en [Vercel](https://vercel.com). El framework Vite se detecta automáticamente.
2.  Agregue la variable de entorno `VITE_API_URL=https://<su-url-de-app>/api` e implemente.

Eso es todo. Regístrese y la vista de inventario cargará los datos de ejemplo.

### Notas del nivel gratuito

- El contenedor de la API se reduce a cero tras la inactividad, por lo que la primera solicitud después de una pausa puede tardar unos segundos.
- La oferta gratuita de Azure SQL se pausa de la misma manera; la lógica de reintentos maneja la reactivación.
- Mantenga `JWT__Key` secreta y única. Cualquiera que la tenga puede forjar tokens de autenticación.
- El flujo de desarrollo local (Docker Compose + SQL Server + `dotnet run`) no cambia.
